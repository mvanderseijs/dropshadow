classdef (ConstructOnLoad = true) dropshadow < matlab.mixin.SetGet
...
%   Version 1.1.
    ...
    methods
        ...
        %Creates the dropshadow object.            
            ...
            assert(isa(handle(h),'matlab.graphics.axis.Axes'),'Parent must be an axes handle.')
            ...
            obj.listeners.ObjectBeingDestroyed = listener(h, 'ObjectBeingDestroyed', @(~,~) delete(obj));            
...
hAxes = obj.ParentAxes;
hFig = obj.ParentFig;

rect = hgconvertunits(hFig, get(hAxes,'position'), get(hAxes,'units'), 'pixels', get(hAxes,'parent')); %x/y coords of fighandle in pixels
rect(1:2)=[1 1];

monitors=get(0,'monitorpositions');
maxheight=max(monitors(:,4)); %max vertical resolution of all monitors

origin=getpixelposition(hAxes,true);

%also need position of the figure containing h, since origin is w.r.t this figure:
figorigin = hgconvertunits(hFig, get(hFig,'position'), get(hFig,'units'), 'pixels', 0); %x/y coords of fighandle in pixels
...
w=floor(rect(3))+1;
hAxes=floor(rect(4))+1;

figure(hFig); % Make sure figure is visible on screen
drawnow;
pause(0.05)

% Java Robot snapshot:
robo = java.awt.Robot;
target = java.awt.Rectangle(x,y,w,hAxes);
image = robo.createScreenCapture(target);

% Get RGBA data from BufferedImage
RGBA32 = image.getRGB(0,0,w,hAxes,[],0,w); 
